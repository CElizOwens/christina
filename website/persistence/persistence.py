...
def get_all_pieces():
    with engine.connect() as con:
        result = con.execute("SELECT c.last_name AS composer, p.title, p.opus, p.date_played, e.ens_type FROM piece p INNER JOIN ensemble e ON p.ensemble_id = e.id INNER JOIN composer c ON p.composer_id = c.id;")
        pieces = []
        for row in result:
